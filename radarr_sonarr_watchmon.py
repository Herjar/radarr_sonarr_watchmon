                ...
                if id == radarr_imdb:
                    ...
                    if self.radarr_delete_file:
                        if "movieFile" not in movie_json.keys():
                            ...
                            request_uri = "http://" + self.radarr_address + "/api/v3/moviefile/" + str(
                                radarr_moviefile_id) + "?apikey=" + self.radarr_apikey
                            ...
                            if trakt_season == sonarr_season and trakt_ep == sonarr_ep:
                                ...
                                if self.sonarr_delete_file:
                                    if "episodeFile" not in sonarr_episode_json.keys():
                                        ...
                                        request_uri = "http://" + self.sonarr_address + "/api/v3/episodefile/" + str(
                                            sonarr_episodefile_id) + "?apikey=" + self.sonarr_apikey
                                        r = requests.delete(request_uri)
                                        if r.status_code != 200:
                                            print("   Error: " + str(r.text))
        ...
        for showid_string in show_episodes:
            ...
            for show in series:
                ...
                if showid == medusa_tvdb:
                    ...
                    medusa_show_eps = requests.get("http://" + self.medusa_address + "/api/v2/series/" + medusa_id + "/episodes?limit=1000",
                    ...
                    for trakt_season_ep in show_episodes[showid_string]:
                        ...
                        for medusa_show_ep in medusa_show_eps:
                            ...
                            if trakt_season == medusa_season and trakt_ep == medusa_ep and medusa_current_status != "Archived" and medusa_current_status != "Ignored":
                                ...
                                medusa_patch = requests.patch("http://" + self.medusa_address + "/api/v2/series/" + medusa_id + "/episodes/" + medusa_epid,
                                    data=medusa_status, headers=headers).json()
